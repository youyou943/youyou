...
export default {
  ...
  methods: {
    ...
    fetchTrainingScores() {
     ...
     const timestamp = Math.floor(new Date(this.trainingTime).getTime() / 1000);
console.log(timestamp); // 输出九位时间戳（秒级）
      //http://10.21.30.30:8080
      return axios.get('http://10.21.30.30:8080/train/getChatScores', {
        params: {
          userId: this.userId,
    ...
    fetchReferenceVersion() {
      //http://10.21.30.30:8080
      axios.get('http://10.21.30.30:8080/train/getComment', {
        params: {
          userId: this.userId,
          time: this.trainingTime
        }
        ...
        .then(response => {    
          ...
          let res = this.formatChatJson(response.data); 
          console.log(res);
          let chat = res.data;
          console.log(chat);
    ...
    formatChatJson(inputData) {
  try {
    // 确保输入是一个对象且包含 'dialogue' 字段
    if (!inputData || !Array.isArray(inputData.dialogue)) {
      throw new Error("No valid 'dialogue' field found in the response data.");
    }
    
    // 返回包含 'chat' 字段的格式化结果
    return {
      chat: inputData.dialogue
    };
  } catch (error) {
    console.error("Error processing JSON:", error.message);
    return { chat: [] };  // 如果处理失败，返回一个空数组作为默认值
  }
},
    // 绘制雷达图
    fetchRadar() {
